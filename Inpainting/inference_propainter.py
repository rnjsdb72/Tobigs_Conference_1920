...
def read_frame_from_videos(frame_root):
    if frame_root.endswith(('mp4', 'mov', 'avi', 'MP4', 'MOV', 'AVI')): # input video path
        ...
        video_name = os.path.basename(frame_root).split(".")[0]
...
def infer(frames, fps, size, video_name, args):
    ...
    save_root = os.path.join(args.output, video_name, "inpainted")
    if not os.path.exists(save_root):
        os.makedirs(save_root, exist_ok=True)

    if args.mode == 'video_inpainting':
        frames_len = len(frames)
        fn_mask = os.path.join(args.output, video_name, "alpha.png")
        flow_masks, masks_dilated = read_mask(fn_mask, frames_len, size, 
    ...
    ##############################################
    ckpt_path = os.path.join(args.checkpoints, 'raft-things.pth')
    fix_raft = RAFT_bi(ckpt_path, device)
    
    ckpt_path = os.path.join(args.checkpoints, 'recurrent_flow_completion.pth')
    ...
    ##############################################
    ckpt_path = os.path.join(args.checkpoints, 'ProPainter.pth')
    ...
    if args.input_type == "image":
        ...
        imageio.imwrite(os.path.join(save_root, 'masked_in.png'), masked_frame_for_save)
        imageio.imwrite(os.path.join(save_root, 'inpaint_out.png'), comp_frames)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-ckpt', '--checkpoints', type=str, default='./ckpt/propainter', help='Path of checkpoints. Default: ./ckpt/propainter')
    parser.add_argument(
        '-i', '--inputs', type=str, default='inputs/object_removal/bmx-trees', help='Path of the input video or image folder.')
    # parser.add_argument(
    ...
    if os.path.isdir(args.inputs):
        files = glob(f"{args.inputs}/*")
        fps = None
        for fn_ in tqdm(files):
            frames_, fps_, size_, video_name_ = read_frame_from_videos(fn_)
            infer(frames_, fps, size_, video_name_, args)
    else:
        frames, fps, size, video_name = read_frame_from_videos(args.inputs)
        infer(frames, fps, size, video_name, args)