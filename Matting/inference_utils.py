...
class ImageSequenceReader(Dataset):
    def __init__(self, path, transform=None, return_fn=True):
        self.path = path
...
class ImagesWriter:
    ...
    def write(self, frames, fns, fn):
        # frames: [T, C, H, W]
        for idx, fn_ in enumerate(fns):
            path_ = os.path.join(self.path, fn_)
            os.makedirs(path_, exist_ok=True)
            to_pil_image(frames[idx]).save(os.path.join(
                path_, fn + '.' + self.extension))
