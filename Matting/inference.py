...
from inference_utils import VideoReader, VideoWriter, ImageSequenceReader, ImageSequenceWriter, ImagesWriter
...
def convert_video(model,
    ...
    if os.path.isfile(input_source):
        return_fn_reader = False
        source = VideoReader(input_source, transform)
    else:
        return_fn_reader = True
        source = ImageSequenceReader(input_source, transform)
    reader = DataLoader(source, batch_size=seq_chunk, pin_memory=True, num_workers=num_workers)
    
    # Initialize writers
    if output_composition is not None:
        writer_com = ImagesWriter(output_composition, "png")
    if output_alpha is not None:
        writer_pha = ImagesWriter(output_alpha, "png")
    if output_foreground is not None:
        writer_fgr = ImagesWriter(output_foreground, "png")
    ...
    try:
        with torch.no_grad():
            ...
            for src in reader:
                ...
                if output_foreground is not None:
                    if return_fn_reader:
                        writer_fgr.write(fgr[0], fns, "foreground")
                    else:
                        writer_fgr.write(fgr[0], "foreground")
                if output_alpha is not None:
                    if return_fn_reader:
                        writer_pha.write(pha[0], fns, "alpha")
                    else:
                        writer_pha.write(pha[0], "alpha")
                if output_composition is not None:
                    ...
                    if return_fn_reader:
                        writer_com.write(com[0], fns, "composition")
                    else:
                        writer_com.write(com[0], "composition")
